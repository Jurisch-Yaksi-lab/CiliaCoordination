function [max_wavelength, bin_size] = cr_patchwise_analysisV3(phase_patch, pos, PowerSpec, check, CBF)
...
[xfield, yfield, phase_pd, phase_sd] = wave_direction(lmatrix); %#ok<ASGLU>
...
[wave_length] = wavelength(lmatrix);
...
% 1) Create a new folder where the checks are saved 
% 2) Have a separate function create all the plots and save them into the
% folder. 
% All plots are: 1) the phase of the patch, 2) the polar histogram, 3)
% Direction per pixel 4) Pixel magnitude 5) colour wheel
% 6) All patches segmentation 7) This patch 
cr_check_per_patch(lmatrix, phase_patch)
...
%% Nested functions
    function [xfield, yfield, phase_pd, phase_sd] = wave_direction(lmatrix)
        ...
        for i = 1:NumPatches
            ...
            theta_patch = wrapTo2Pi(theta_patch(~isnan(theta_patch))); % shift the phases to [0, 2pi] interval
            ...
            phase_pd(mask_patch) = mean_phases(i); 
            phase_sd(mask_patch) = cv_phases(i);
            
            % Plot polarplots and directions per patch to check for
            figure(1), subplot(7,6,i), polarhistogram(theta_patch,binEdges), title(sprintf('mean_phase = %0.2f, sv = %0.2f', mean_phases(i), cv_phases(i)));
            figure(2), subplot(7,6,i), imagesc(phase_pd, 'AlphaData', mask_patch), title(sprintf('mean_phase = %0.2f, sv = %0.2f', mean_phases(i), cv_phases(i)));
            set(gca,'XTickLabel',get(gca,'XTick')*CBF.spatres);
            set(gca,'YTickLabel',get(gca,'YTick')*CBF.spatres); set(gca,'YDir','normal');
